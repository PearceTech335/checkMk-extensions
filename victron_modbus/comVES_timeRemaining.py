...
def main():
    ...
    points = {
        846: ("time_to_go", "Estimated Time to Discharge", 0.01, "hours", None, None, False)
    }
    ...
    for addr, (id_name, display_name, scale, unit, warn, crit, is_signed) in points.items():
        ...
        if isinstance(raw, list):
            ...
            val = raw_val / scale
            # this can probably be cleaned up with just a / 3600 if scaling works correctly - but will remain for all eternity as it is because maths is hard and i am lazy
            val = val / 60 #minutes
            val = val / 60 #hours
            val = round(val, 2)
            if val == 0:
                print(f"System Charging - Estimated Time Left calculated during discharge cycle")
            else:
                perf = f"{id_name}={val};;;;"
                summary = f"{val} {unit}"
