...
def main():
    ...
    # Set LOW Battery levels here...
    points = {
        840: ("batt_volts", "Battery Voltage", 10, "V", 47.5, 44.5, False),
    }
    ...
    for addr, (id_name, display_name, scale, unit, warn, crit, is_signed) in points.items():
        ...
        if isinstance(raw, list):
            ...
            val = raw_val / scale
#            val = 42
            # Set min/max values of the perf data histogram with this 20;80 - change it to suit your battery system
            perf = f"{id_name}={val};{warn};{crit};20;80"
            summary = f"{val} {unit}"

            # Threshold Check
            # This threshold checking still has to be fleshed out - at present the critical and warning levels are displayed in ther perf data
            # and shown correctly in the histogram, but no alerting can be configured as the state change from OK/WAARN/CRIT is what triggers the alert
            if val <= crit:
                service_state = 2
            elif val <= warn:
                service_state = 1
